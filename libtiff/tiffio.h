 ...
 *
 * Till now TIFFDoubleToRational() is defined in tif_dirwrite.c locally, but need to be used also in tif_dirread.c and others.
 * Also the user will need DoubleToRational() to calculate numerator and denominator for rationals.
 * Therefore, also TIFFDoubleToRational() has to be added to tiflib.def.
...
*/
extern void TIFFDoubleToRational(double value, uint32_t* num, uint32_t* denom);
extern void TIFFDoubleToSrational(double value, int32_t* num, int32_t* denom);
